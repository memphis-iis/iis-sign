...
function SimulationRenderer( WIDTH, renderer ) {
	...
	function generatePositionTexture() {
		...
		for (var k = 0, kl = a.length; k < kl; k += 4) {
			a[k + 0] = Math.random() * BOUNDS - BOUNDS_HALF;  // x
			a[k + 1] = Math.random() * BOUNDS - BOUNDS_HALF;  // y
			a[k + 2] = Math.random() * BOUNDS - BOUNDS_HALF;  // z
			a[k + 3] = 1;
		}
	...
	function generateVelocityTexture() {
		...
		for (var k = 0, kl = a.length; k < kl; k += 3) {
			a[k + 0] = (Math.random() - 0.5) * 10;  // x
			a[k + 1] = (Math.random() - 0.5) * 10;  // y
			a[k + 2] = (Math.random() - 0.5) * 10;  // z
		}
